...
class MapConstructor:
    ...
    area - область, в которой появится робот.
    Эта область должна быть свободной от препятсивий.
    area = {
            'x_left': x_left,
            'x_right': x_right,
            'y_top': y_top,
    ...
    def __init__(self, map_width, map_length, hurdles_max_size, hurdles_count,
        ...
        if any(not isinstance(arg, int) for arg in
               [map_width, map_length, hurdles_max_size, hurdles_count]):
            raise TypeError(f'Expected int arguments')

        if not isinstance(area, dict):
            raise TypeError(f'Expected dict object')
        ...
        self.hurdles = hurdle_fabric(map_width, map_length, hurdles_max_size,
                                     hurdles_count)
        self.hurdles_points = []
        self._hurdle_char = '#'
    ...
    def get_hurdlers_points(self, area):
        ...
        for item in self.hurdles:
            for point in item.get_body_coord():
                if not (area['x_left'] < point[0] < area['x_right'] and area[
                    'y_bottom'] < point[1] < area['y_top']):
                    res.append(point)
        res += body.Body(0, 0, self.map_width,
                         self.map_length).get_body_coord()
