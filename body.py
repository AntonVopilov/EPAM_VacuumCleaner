class Body:
    ...
    def calculate_body_coord(self):
        ...
        for i in range(self.width):
            ...
            self._bottom_bounder.append(
                [self.x_left_top + i, self.y_left_top + self.length - 1])
        for i in range(1, self.length):
            self._right_bounder.append(
                [self.x_left_top + self.width - 1, self.y_left_top + i])
    ...
    def get_body_center(self):
        """
        Метод возвращает положение центра прямоугольника
        (x_center: int, y_center: int)
        """
        return self.x_left_top + self.width // 2, \
               self.y_left_top + self.length // 2
    ...
    def add_body_to_scr(self, stdscr, char):
        ...
        Обновление stdscr не выполняется,
        чтобы обновить вызови stdscr.refresh(*args)
        """
